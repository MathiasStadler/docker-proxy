...
while true; do
    rule_num=$(sudo iptables -t nat -L PREROUTING -n --line-numbers |
        grep -E "ACCEPT.*tcp.*$SQUIDIP.*tcp.*dpt.*:80.*" |
        awk '{print $1}' |
        head -n1)

    [ -z "$rule_num" ] && break
    echo -en "DELETE RULE => "
    sudo iptables -t nat -L PREROUTING "${rule_num}"
...
while true; do
    ...
    rule_num=$(sudo iptables -t nat -L PREROUTING -v -n --line-numbers |
...
while true; do
    ...
    rule_num=$(sudo iptables -t nat -L POSTROUTING -v -n --line-numbers |
        grep -E 'MASQUERADE.*all.*--.*' |
        awk '{print $1}' |
        head -n1)

    [ -z "$rule_num" ] && break
    echo -en "DELETE RULE => "
    sudo iptables -t nat -L POSTROUTING "${rule_num}"
...
# TODO old echo "DROP.*tcp.*$external_interface.*tcp.*dpt:$SQUIDPORT"
    ...
    [ -z "$rule_num" ] && break
    echo -en "DELETE RULE => "
    sudo iptables -t mangle -L PREROUTING "${rule_num}" || error_rule_number ${rule_num}
